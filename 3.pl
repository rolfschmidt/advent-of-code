...
sub CalcWires {
    ...
    for my $WireIndex (0 .. $#Wires) {
        my $Route = $Wires[$WireIndex];
        $X = 0;
        $Y = 0;
        $WireSteps = 0;

        my @SplitRoute = split /\s*,\s*/, $Route;
        for my $Command (@SplitRoute) {
            my $Direction = substr $Command, 0, 1;
            my $Steps     = substr $Command, 1;
            ...
            for (1 .. $Steps) {
                ...
                $WireSteps += 1;
                if ( !defined $Matrix{$X}->{$Y}->{$WireIndex} ) {
                ...
                next STEP if %{ $Result || {} } && $Result->{Distance} < $Distance && $Type eq 'Distance';
...
my $Result = CalcWires(<<WWIRE
R8,U5,L5,D3
U7,R6,D4,L4
WWIRE
);

die "Test 1 failed - $Result->{Distance}" if $Result->{Distance} != 6;

$Result = CalcWires(<<WWIRE
R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83
WWIRE
);

die "Test 2 failed - $Result->{Distance}" if $Result->{Distance} != 159;
...
die "Test 3 failed - $Result->{Distance}" if $Result->{Distance} != 135;
...
die "Puzzle failed - $Result->{Distance}" if $Result->{Distance} != 207;
...
die "Puzzle Part 2 - Test 1 failed - $Result->{StepsTotal}" if $Result->{StepsTotal} != 610;
...
die "Puzzle Part 2 - Test 2 failed - $Result->{StepsTotal}" if $Result->{StepsTotal} != 410;
...
die "Puzzle Part 2 - puzzle - $Result->{StepsTotal}" if $Result->{StepsTotal} != 21196;
